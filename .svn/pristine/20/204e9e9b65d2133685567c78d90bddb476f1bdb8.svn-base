...
$(function(){	
		 		   ...
		 		   var args = {"date_begin":begin_day_class,"date_end":end_day_class,"time": new Date()};
		 		   		$.post(url,args,
		 		    	     ...
		 		    	     for(var i = 0; i < data_class.length; i++){
		 		    	    	 ...
		 		    	    	 if(i == data_class.length - 1){//合计
		 		    	    		 
		 		    	    		var newTr = "<tr><td>"+"合计"+"</td><td>"+racl.toFixed(2)+"</td><td>"+rhd.toFixed(2)+"</td><td>"+rhq.toFixed(2)+"</td><td>"
	...
	function click_dayban(){
		   ...
		   var end_day_class = $("#indate_day_ban2").val();
			 ...
			 var data33 = [];//数据3
			 /*班合计*/
			 var total_racl = [0,0,0];//日安产量
			 var total_rhd = [0,0,0];//日耗电
			 var total_rhq = [0,0,0];//日耗气
			 var total_ybs = [0,0,0];//样本数
			 /*var total_dazhxh = [0,0,0];//吨氨综合消耗
			 var total_dadh = [0,0,0];//吨氨电耗
			 var total_daqh = [0,0,0];//吨氨气耗数
*/			 
			 var x_zuobiao = [];//横坐标 时间字符串转化而来
			 if(begin_day_class != "" &&  end_day_class != "" ){
			   if(begin_day_class > end_day_class){
			   ...
			   var args = {"date_begin":begin_day_class,"date_end":end_day_class,"time": new Date()};
			   		$.post(url,args,function(data){
			    	     ...
			    	     for(var i = 0; i < data_class.length; i++){
					    	 ...
					    	 var rhd     = data_class[i].time_electric_consume;//日耗电
					    	 var rhq     = data_class[i].time_raw_gas_consume;//日耗气
					    	 var xsacl   = (racl==0)?0:(racl / orig_id * 6);//小时氨产量
					    	 var dazh    = (racl==0)?0:(1.2143 * rhq / racl + rhd / racl * 0.1229);//吨氨综合消耗
					    	 var dadh    = (racl==0)?0:(rhd / racl);//吨氨电耗
					    	 var daqh    = (racl==0)?0:(rhq / racl);//吨氨气耗
			    	    	 var ban     = parseInt(data_class[i].classNumber);//早中晚班
			    	    	 var newdate = new Date();
			    	    	 var n_time  = renderTime(time.toString(),true);
			    	    	 var class_number = 0;
			    	    	 if(data_class[i].shift!=0||orig_id!=0){
			    	    		 class_number = parseInt(data_class[i].shift/orig_id);//123
			    	    	 }
			    	    	 switch(class_number)
			    	    	 {
			    	    	 case 1:
			    	    		 total_racl[0] += racl;//日安产量
			    				 total_rhd[0]  += rhd;//日耗电
			    				 total_rhq[0]  += rhq;//日耗气
			    				 total_ybs[0]  += orig_id;//样本数
			    				 break;
			    	    	 case 2:
			    	    		 total_racl[1] += racl;//日安产量
			    				 total_rhd[1]  += rhd;//日耗电
			    				 total_rhq[1]  += rhq;//日耗气
			    				 total_ybs[1]  += orig_id;//样本数
			    				 break;
			    	    	 case 3:
			    	    		 total_racl[2] += racl;//日安产量
			    				 total_rhd[2]  += rhd;//日耗电
			    				 total_rhq[2]  += rhq;//日耗气
			    				 total_ybs[2]  += orig_id;//样本数
			    				 break;
			    	    	 }
			    	    	 /*数据 分班*/
			    	    	 switch(ban)
			    	     ...
			    	     var totalTr;
			    	     for(var i = 0; i <=2; i++){
			    	    	 if(total_racl[i].toFixed(2) == "0.00"){
			    	    		 totalTr += "<tr><td>"+"合计"+"</td><td>"+eval(i+1)+" 班"+"</td><td>"
				    	    	 +"--"+"</td><td>"
				    	    	 +"--"+"</td><td>"
				    	    	 +"--"+"</td><td>"
				    	    	 +"--"+"</td><td>"
				    	    	 ...
				    	    	 totalTr += "<tr><td>"+"合计"+"</td><td>"+eval(i+1)+" 班"+"</td><td>"
				    	    	 +total_racl[i].toFixed(2)+"</td><td>"
				    	    	 +total_rhd[i].toFixed(2)+"</td><td>"
				    	    	 +total_rhq[i].toFixed(2)+"</td><td>"
		 		    	    	 +(total_racl[i]/total_ybs[i] *6).toFixed(2)+"</td><td>"
		 		    	    	 +Math.round(eval(1.2143 * total_rhq[i]/total_racl[i] + total_rhd[i]/total_racl[i] * 0.1229) * 100) /100+"</td><td>"
		 		    	    	 +(total_rhd[i]/total_racl[i]).toFixed(2)+"</td><td>"
		 		    	    	 +(total_rhq[i]/total_racl[i]).toFixed(2)+"</td><td>"
		 		    	    	 +parseInt(total_ybs[i])+"</td></tr>";
			    	    	 }
			    	     }
			    	     //alert(totalTr)
			    	     $("#energy_day_ban").append(totalTr);
			    	     /*合并行*/
			   ...
			   var args = {"mouth_date_begin":begin_month_class,"time": new Date()};
			   		$.post(url,args,function(data){
			    	     ...
			    	     for(var i = 0; i < data.length; i++){
			    	    	 ...
			    	    	 ban = parseInt(ban);
			    	    	 var class_number = 0;
			    	    	 if(data[i].shift!=0||orig_id!=0){
			    	    		 class_number = parseInt(data[i].shift/orig_id);//123
			    	    	 }
			    	    	 switch(class_number)
			    	    	 {
			    	    	 case 1:
			    	     ...
			    	     //alert(totalTr)
			    	     $("#energy_month_ban").append(totalTr);
			    	     /*合并行*/
			  		     /*_w_table_rowspan("#energy_day_ban",1);*/
			  		     _w_table_rowspan("#energy_month_ban",1);
				   ...
				   $.post(url,args,function(data){
			    	   //alert(data)
			    	     ...
			    	     for(var i = 0; i < data.length; i++){
			    			 ...
			    			 data5.push([renderTime(time.toString(),true).substring(0, 7),Math.round(rhd*100)/100]) ;       //数据2  月耗气(Nm3)
			    			 data6.push([renderTime(time.toString(),true).substring(0, 7),Math.round(rhq*100)/100]) ;       //数据2  月耗电(kgce)
  ...
  function myScroll(tbl_hd,rel_tbl) {
	...
	maxH = 30 * (realL-1) + 100;
  	...
  	if (doc.scrollTop() >= 100) { //大于头部高度
  		$("."+tbl_hd).css("visibility","visible");
  	} else if (doc.scrollTop() <= 100) { //相反
