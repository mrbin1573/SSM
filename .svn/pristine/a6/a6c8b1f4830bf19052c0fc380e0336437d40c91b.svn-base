...
public class ValidateColorServlet extends HttpServlet {
	...
	public void init(){
		fontHeight = height - 2;
		codeX = width / (codeCount );//设置验证码相互之间的间距
		codeY = height - 4;
	}
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		graphics.setColor(Color.orange);//验证码的背景颜色
		...
		for(int i = 0; i < codeCount; i++){
			...
			//用随机产生的颜色将验证码绘制到图像中
			if(i==1){
				graphics.setColor(Color.white);//验证码字体颜色
			}
			if(i==2){
				graphics.setColor(Color.red);//验证码字体颜色
			}
			if(i==3){
				graphics.setColor(Color.BLUE);//验证码字体颜色
			}
			if(i==0){
				graphics.setColor(Color.CYAN);//验证码字体颜色
			}
			
			graphics.drawString(strRand, (i + 0)* codeX, codeY);//设置验证码的位置
		}
		//再将存放所有的随机字符的StringBuffer，对应放到Httpsession中
