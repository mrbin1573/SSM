                ...
                <div class="tab-content menu-third-content navbar-fixed-top">
                    <div class="tab-pane fade in active" id="three_1">
                        ...
                        <ul id="myTab" class="nav nav-tabs menu_rihgt_after">
                            <li class="active">
                                <a href="#three_c_1" data-toggle="tab"><img src="images/menu_third.png">&nbsp;&nbsp;原始数据上传</a>
                            </li>
                        </ul>
                        </div>
                </div>
                                    ...
                                    <form id="uploadForm">
	                                    <div class="upload">
	                                        <a href="javascript:;" class="file">选择 excel 文件
	                                                <input type="file" name="jf_file" id="file"  onchange="fileList()" accept="application/vnd.ms-excel" multiple>
	                                        		
	                                        </a>
	                                        <c:if test="${db_endtime != null}">
	                                        	<span>&nbsp;温馨提示：您已上传的数据时间至${db_endtime}，请上传${db_endtime}以后的原始数据。</span>
	                                        </c:if>
    ...
    //function progressGo() {
     //   var p = $(".progress-bar"); /*获取进度条*/
      ...
      //  p.each(function () {
       ...
       // })
   // }${pageContext.request.contextPath}/doAjaxUpload.action"
        ...
        function UpladFile() {
            ...
            var url = "${pageContext.request.contextPath}/qt_doUploadFile.action"; // 接收上传文件的后台地址 
        ...
        function uploadComplete(evt) {
         //服务断接收完文件返回的结果
         $(window.parent.document).find(".czwc").text("操作完成，即将关闭！");
		 setTimeout(function(){$(window.parent.document).find('.jindutiao').css('display','none');tohref();},0);
        
        }
        function tohref(){
        	window.location.href ="qt_toUploadType.action";
        }
        //上传失败
